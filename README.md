...
1. Search for `'fiveanddime'` (inside single quotations) to capture the text domain.
2. Search for `fiveanddime_` to capture all the function names.
3. Search for <code>&nbsp;_tk</code> (with a space before it) to capture DocBlocks.
4. Search for `fiveanddime-` to capture prefixed handles.

OR

* Search for: `'fiveanddime'` and replace with: `'megatherium'`
* Search for: `fiveanddime_` and replace with: `megatherium_`
